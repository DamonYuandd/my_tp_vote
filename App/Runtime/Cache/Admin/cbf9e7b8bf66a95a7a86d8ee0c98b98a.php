	...
	<div class="header-title" style="background-color:#0e0e0d;">
        ...
        <div id="top_nav_box">
            ...
            <ul class="top-nav">
                ...
                <?php if(is_array($moduleList)): $i = 0; $__LIST__ = $moduleList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><?php
						if( !empty($vo['burl']) ) {
							$main_url = $vo['burl'];
						} else {
					...
					?>
                	<li><a href="javascript:void(0)" id="<?php echo ($vo["alias"]); ?>" onClick="goToModule(this,'<?php echo $main_url;?>','Index/sidemenu/pid/<?php echo ($vo["id"]); ?>/cid/<?php echo ($vo["first_cid"]); ?>/lang/<?php echo ($vo["lang"]); ?>')"><?php getDefNavTitle(json_decode($vo['title'],true),$_SESSION['lang']);?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
