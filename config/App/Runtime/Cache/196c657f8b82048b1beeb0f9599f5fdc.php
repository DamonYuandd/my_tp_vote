    ...
    <div class="accordion_child">
    	...
    	<table class="" cellspacing="5">
    		<tr>
				...
				<td><input name="website_name" id="website_name" value="<?php echo ($account["website_name"]); ?>"></td>
    		</tr>
			<tr>
				...
				<td>
					...
					?>
					<textarea name="url" id="url" rows="3" style="width:251px"><?php echo ($url_str); ?></textarea> 
				<div style="color:#999">不带http://www. ,首个域名必需赠送的三级域名，每个域名结尾要有逗号","。</div><br></td>
    		</tr>
			<tr>
    			<td align="right"></td>
				<td><input name="yy_price" id="yy_price" value="<?php echo ($account["yy_price"]); ?>"></td>
    		</tr>
			<tr>
    			<td align="right">服务包:</td>
				<td><input name="service_price" id="service_price" value="<?php echo ($account["service_price"]); ?>"></td>
    		</tr>
			<tr>
    			<td align="right">合同号:</td>
				<td><input name="contract_number" id="contract_number" value="<?php echo ($account["contract_number"]); ?>"></td>
    		</tr>
    		<tr>
    ...
    <div class="accordion_child">
		...
		<table class="database" cellspacing="5">
			...
			<tr>
				...
				<td><input value="<?php echo ($account["contract_number"]); ?>" disabled="false">
					<input type="hidden" name="db_name" id="db_name" value="<?php echo ($account["contract_number"]); ?>"></td>
    		</tr>
			<tr>
  ...
  <div id="test2-content">
    <div class="accordion_child">
    	<table id="lang_table" class="language_table" cellspacing="5" width="100%">
			<tr><td><h3>电脑版</h3></td><td><a href="javascript:;" onclick="parent.goToMainFrame('__APP__/Lang/edit/account_id/<?php echo ($account["id"]); ?>/pid/3');">添加语言</a></td></tr>
    		<tr>
    			<th>状态</th>
    			<th>语言名称</th>
				<th>别名</th>
			...
			<?php if(is_array($langList)): $i = 0; $__LIST__ = $langList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><?php if(($vo['pid'])  ==  "3"): ?><tr>
				<th><input type="checkbox" name="pc_langs" value="<?php echo ($vo["is_publish"]); ?>" disabled="false"></th>
    			<td style="<?php if($vo['is_default']==1){echo 'color:red;';} ?>"><?php echo ($vo["title"]); ?></td>
				<td><?php echo ($vo["alias"]); ?></td>
				<td><?php echo ($vo["ordernum"]); ?></td>
				<td><a href="javascript:;" onclick="parent.goToMainFrame('__APP__/Lang/edit/pid/3/account_id/<?php echo ($account["id"]); ?>/id/<?php echo ($vo["id"]); ?>');">修改</a>
				 | <a href="javascript:;" name="lang_delete" id="<?php echo ($vo["id"]); ?>">删除</a></td>
    		</tr><?php endif; ?><?php endforeach; endif; else: echo "" ;endif; ?>
			<tr>
    			<th colspan="5" valign="top" align="right" height="30"></th>
    		</tr>
			<tr><td><h3>手机版</h3></td><td><a href="javascript:;" onclick="parent.goToMainFrame('__APP__/Lang/edit/account_id/<?php echo ($account["id"]); ?>/pid/4');">添加语言</a></td></tr>
			<?php if(is_array($langList)): $i = 0; $__LIST__ = $langList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><?php if(($vo['pid'])  ==  "4"): ?><tr>
				<th><input type="checkbox" name="m_langs" value="<?php echo ($vo["is_publish"]); ?>" disabled="false"></th>
    			<td style="<?php if($vo['is_default']==1){echo 'color:red;';} ?>"><?php echo ($vo["title"]); ?></td>
				<td><?php echo ($vo["alias"]); ?></td>
				<td><?php echo ($vo["ordernum"]); ?></td>
				<td><a href="javascript:;" onclick="parent.goToMainFrame('__APP__/Lang/edit/pid/4/account_id/<?php echo ($account["id"]); ?>/id/<?php echo ($vo["id"]); ?>');">修改</a>
				 | <a href="javascript:;" name="lang_delete" id="<?php echo ($vo["id"]); ?>">删除</a></td>
    		</tr><?php endif; ?><?php endforeach; endif; else: echo "" ;endif; ?>
			<tr>
  ...
  <div id="test3-content">
    <div class="accordion_child">
    	<table class="language_table" cellspacing="5" width="100%">
    		<tr>
    			<td><h3>导航栏</h3></td>
				<td><a href="javascript:;" onclick="parent.goToMainFrame('__APP__/Category/editNav/account_id/<?php echo ($account["id"]); ?>');">添加导航</a></td>
			</tr>
    		<tr>
    			<th>状态</th>
				<th>导航名称</th>
			...
			<?php if(is_array($moduleList)): $i = 0; $__LIST__ = $moduleList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				<th><input type="checkbox" name="module_publish" value="<?php echo ($vo["is_publish"]); ?>" disabled="false"></th>
				<td><?php getDefNavTitle(json_decode($vo['title'],true),$_SESSION['c_lang']);?></td>
				<td><?php getModelTitle($vo['module']);?></td>
				<td><?php echo ($vo["ordernum"]); ?></td>
				<td><a href="javascript:;" onclick="parent.goToMainFrame('__APP__/Category/editNav/account_id/<?php echo ($account["id"]); ?>/id/<?php echo ($vo["id"]); ?>');">修改</a>
				 | <a href="javascript:;" name="module_delete" id="<?php echo ($vo["id"]); ?>">删除</a></td>
			...
			<tr>
				...
				<td>
					<select id="module_category" name="module_category" onclick="changeCategory(this,'one_category')" multiple="multiple" style="height:150px;width:235px">
						<?php if(is_array($moduleList2)): $i = 0; $__LIST__ = $moduleList2;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php getDefNavTitle(json_decode($vo['title'],true),$_SESSION['c_lang']);?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
				</td>
    		</tr>
    		<tr>
