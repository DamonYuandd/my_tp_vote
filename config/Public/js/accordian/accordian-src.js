...
function Accordian(){
	...
	this.init = function(d,s,tc) {
		// get all the elements that have id as content
		l=$(d).getElementsByTagName('div');
		this.c=[];
		for(i=0;i<l.length;i++){
			h=l[i].id;
			if(h.substr(h.indexOf('-')+1,h.length)=='content'){this.c.push(h);}
		}
		sel=null;
		...
		for(i=0;i<l.length;i++){
			...
			if(h.substr(h.indexOf('-')+1,h.length)=='header'){
				...
				h.onclick = function(){
					if( window.formValidation()==false ) {
						return false;
					}
	...
	this.openTab = function(sn) {
		for(i=0;i<this.c.length;i++){
			cn=this.c[i];
			n=cn.substr(0,cn.indexOf('-'));
			if((n+'-header')==sn+'-header'){
				ex($(n+'-content'));
				n=$(n+'-header');
				cc(n,'__');
				n.className=n.className+' '+n.tc;
	...
	this.nextTab = function() {
		for(i=0;i<this.c.length;i++){
			cn=this.c[i];
			n=cn.substr(0,cn.indexOf('-'));
			hc = jQuery('#'+n+'-header').attr('class');
			hi = hc.indexOf('header_highlight');
			if( hi>0 ) {
