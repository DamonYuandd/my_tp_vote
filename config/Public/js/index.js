...
$(function(){
	...
	$("#on_ordernum").click(function(){
		if(window.tipCheckbox()) {
			$("#form_list").attr('action',data._APP_+'/'+data.actionName+'/ordernum');
			$("#form_list").submit();
		}
	});
	...
	$("#on_move").click(function(){
		if(window.tipCheckbox()) {
			...
			$("#form_list").attr('action',data._APP_+'/'+data.actionName+'/move');
	...
	$("#on_copy").click(function(){
		if(window.tipCheckbox()) {
			...
			$("#form_list").attr('action',data._APP_+'/'+data.actionName+'/copy');
			window.openCategoryBox();
		}
	});
	...
	$("#on_delete").click(function(){
		if(window.tipCheckbox()) {
			if(confirm('确认删除勾选的数据吗？删除后无法恢复！')) {
				$("#form_list").attr('action',data._APP_+'/'+data.actionName+'/delete');
				$("#form_list").submit();
			} else {
				return false;
			}
...
function updateField(action,id, fval) {
	var url =data._APP_+'/'+data.actionName+'/'+action+'/id/'+id+'/fval/'+fval;
...
function getSearchUrl() {
	return data._APP_+'/'+data.actionName+'/index/rowpage/'+$('#rowpage').val()+'/searchKey/'+$('#searchKey').val();
}
...
function _selectCategoryByPid(element_id, $id, $lang ) {
	$.getJSON(data._APP_+"/Index/selectCategoryByPid",{"pid":$id,"lang":$lang},function(json){
		$('#'+element_id).html('');
		if( json.list==null ) {
			$('#'+element_id).hide();
		} else {
